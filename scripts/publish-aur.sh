...
# --- Stage PKGBUILD/.SRCINFO with that pkgver and exact parent commit -------
# We avoid `makepkg --printsrcinfo` here because this script needs to run on
# hosts without pacman/makepkg. The two fields patched below are deterministic
# projections of the PKGBUILD metadata, so keeping them in lockstep is enough.
...
SOURCE_URL="git+https://github.com/bernardopg/SGI.git#commit=${PARENT_SHA}"
sed -i -E "s|^source=.*|source=('${SOURCE_URL}')|" "$STAGE_DIR/PKGBUILD"
sed -i -E "s|^\tsource = .*|\tsource = ${SOURCE_URL}|" "$STAGE_DIR/.SRCINFO"
...
if ! grep -Fq "source=('${SOURCE_URL}')" "$STAGE_DIR/PKGBUILD"; then
    echo "publish-aur.sh: PKGBUILD source was not pinned correctly" >&2
    exit 1
fi
